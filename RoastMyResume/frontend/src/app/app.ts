...
onFileSelected(event: any) {
  ...
  //   this.http.post<any>('/api/Roast/roast-hidden', formData)
  ...
  // }
  uploadResume() {
  const blob = new Blob(["Test resume content"], { type: 'application/pdf' });
  const dummyFile = new File([blob], "dummy.pdf");
  ...
  this.http.post<any>('/api/Roast', formData)
    .subscribe({
      ...
      error: (err) => {
        console.error("Error during dummy upload:", err);
        this.roastResult = "Dummy upload failed.";
      }
    });
}
